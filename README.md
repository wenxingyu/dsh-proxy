...
- **原生 Basic Auth（默认关闭）**：使用**浏览器自带的认证弹窗**，没有自绘登录页、没有会话 Cookie；默认用户名与密码均为空 = 局域网**开放访问**，在设置页**同时设置**用户名和密码后启用（含 WebSocket 握手）
...
浏览器访问 `http://<本机局域网IP>:3081` → 浏览器弹出原生 Basic Auth 登录框（未设账号密码时直接进入）→ 进入 DSH 界面。凭据与会话由浏览器记住，下次直接进入。
